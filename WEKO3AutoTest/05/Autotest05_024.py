...
def run(playwright):
    ...
    page.click('//*[@id="detail_search_main"]/span[1]')
    ...
    # Fill input[placeholder="Enter Input Value"]
    page.fill("input[placeholder=\"Enter Input Value\"]", "登録テスト")

    page.wait_for_timeout(int(SET_WAIT))

    page.screenshot(path=f'{path.splitext(path.basename(__file__))[0]+"_2"}_capture.png')

    page.click('//*[@id="detail-search-btn"]')
