...
def run(playwright):
    ...
    page.select_option("//div[starts-with(normalize-space(.), 'conference paperdata paperdepartmental bulletin papereditorialjournal articlenew')]/select", "string:patent")
    ...
    # Click text=/.*Source Identifier.*/
    page.click("text=/.*Source Identifier.*/")

    # Press ArrowDown
    page.press("//div[normalize-space(.)='PISSNEISSNISSNNCID']/select", "ArrowDown")
    page.press("//div[normalize-space(.)='PISSNEISSNISSNNCID']/select", "ArrowDown")
    page.press("//div[normalize-space(.)='PISSNEISSNISSNNCID']/select", "ArrowDown")
    ...
    # Click text=/.*JaLC CrossRef DOI.*/ >> input[name="identifier_grant"]
    page.click("text=/.*JaLC CrossRef DOI.*/ >> input[name=\"identifier_grant\"]")
    ...
    page.click("text=/.*Identifier.*/")

    page.wait_for_timeout(int(SET_WAIT))

    page.screenshot(path=f'{path.splitext(path.basename(__file__))[0]+"_6"}_capture.png')
